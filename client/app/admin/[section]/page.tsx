...
> = {
  users: {
    ...
    columns: [
      ...
      { key: "role", label: "Vai trò", render: (row) => formatRole(row.role) },
      {
        key: "accountStatus",
        label: "Trạng thái",
        render: (row) => formatStatus(row.accountStatus),
      },
      {
        key: "isVerified",
        label: "Email xác thực",
        render: (row) => (row.isVerified ? "Có" : "Không"),
      },
  ...
  pets: {
    title: "Thú cưng",
    description:
      "Xem toàn bộ hồ sơ, giấy tờ xác minh và xử lý yêu cầu duyệt tại một nơi.",
  ...
  "system-profile": {
    ...
    description:
      "Quản lý nội dung giới thiệu và thông tin liên hệ hiển thị trên các dịch vụ PetMatching.",
  ...
  "store-overview": {
    ...
    description:
      "Theo dõi doanh thu, đơn hàng, tồn kho và hiệu quả sản phẩm của PetMatching Store.",
  ...
  "store-products": {
    title: "Sản phẩm",
    description:
      "Giám sát danh mục, giá bán, tồn kho và trạng thái sản phẩm của PetMatching Store.",
  ...
  "store-orders": {
    title: "Đơn hàng",
    description:
      "Theo dõi toàn bộ đơn hàng của PetMatching Store ở chế độ chỉ xem.",
  ...
  "spa-overview": {
    title: "Tổng quan Spa",
    description:
      "Theo dõi doanh thu, lịch hẹn, dịch vụ và năng lực vận hành của Spa PetMatching.",
  ...
  "spa-bookings": {
    title: "Lịch đặt spa",
    description:
      "Theo dõi lịch hẹn, thanh toán và chất lượng vận hành Spa ở chế độ chỉ xem.",
  ...
  reports: {
    ...
    description:
      "Tiếp nhận và xử lý các phản ánh phát sinh trong quá trình ghép đôi và trò chuyện.",
        ...
        >
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-1.5 text-3xl font-black tracking-normal text-[#172033]">
                {config.title}
              </h2>
                ...
                <thead className="bg-[#F7F9FB]">
                  <tr>
                    {config.columns.map((column) => (
                      <th
                        key={column.key}
                        className="px-5 py-4 text-[11px] font-black uppercase tracking-wider text-[#64748B]"
                      >
                        {column.label}
                      </th>
                    ))}
                    {hasActions && (
                      <th className="px-5 py-4 text-right text-[11px] font-black uppercase tracking-wider text-[#64748B]">
                        Thao tác
                      </th>
                    )}
                  </tr>
                  ...
                  {paginatedRows.map((row, index) => (
                    ...
                    >
                      {config.columns.map((column) => (
                        <td
                          key={column.key}
                          className="max-w-[280px] truncate px-5 py-4 text-sm font-semibold text-[#334155]"
                        >
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain pr-2">
              <div className="rounded-xl border bg-[#F8FAFC] p-4 text-sm">
                <p className="mb-3 text-[11px] font-black uppercase tracking-wider text-[#64748B]">
                  Nội dung phản ánh
                </p>
                ...
                <div className="mt-4 rounded-lg border border-[#E5EAF0] bg-white p-3">
                  <p className="text-[11px] font-black uppercase tracking-wider text-[#64748B]">
                    Mô tả của người phản ánh
                  </p>
                  <p className="mt-2 whitespace-pre-wrap break-words font-semibold text-[#334155]">
              ...
              <div>
                <h3 className="mb-1 text-sm font-black text-[#172033]">
          ...
          <thead className="bg-[#FAF8F5]">
            <tr className="h-12">
              {[
                ...
                <th
                  key={label}
                  className={`h-12 px-4 align-middle text-[11px] font-black uppercase tracking-[0.08em] text-[#64748B] ${index === 0 ? "text-left" : "text-center"}`}
                >
                  {label}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-[#E5EAF0]">
            {paginatedUsers.map((user) => (
              <tr key={user.id} className="h-20 transition hover:bg-muted/15">
                <td className="px-4 py-3 align-middle">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 flex min-w-0 items-center gap-1.5 truncate text-xs font-semibold text-[#64748B]">
                ...
                <td className="whitespace-nowrap px-4 py-3 text-center align-middle text-sm font-semibold text-[#64748B]">
                  {dateCell(user)}
                </td>
                <td className="px-4 py-3 align-middle">
                  <ActionGroup
...
function hasActionablePetDocument(pet: Row) {
  return Boolean(
    pet.documents?.some((document: Row) =>
...
function hasApprovedPetDocument(pet: Row) {
  return Boolean(
    pet.documents?.some((document: Row) => document.status === "APPROVED"),
  );
}

function hasRejectedPetDocument(pet: Row) {
  return Boolean(
    pet.documents?.some((document: Row) => document.status === "REJECTED"),
  );
}
          ...
          <thead className="bg-[#F7F9FB]">
            <tr className="h-[68px]">
              {["Dịch vụ", "Mô tả"].map((label) => (
                <th
                  key={label}
                  className="align-middle px-4 py-3 text-[11px] font-black uppercase tracking-wider text-[#64748B]"
                >
                  {label}
                </th>
              ))}
            ...
            {paginatedGroups.map((group) => {
              ...
              return (
                <tr key={group.key} className="transition hover:bg-[#FAFBFC]">
                  ...
                  <td className="px-4 py-4 text-sm font-bold text-[#475569]">
                    {duration}
                  </td>
                  <td className="px-4 py-4">
                    <span
                  ...
                  <td className="px-4 py-4 text-sm font-black text-[#172033]">
              ...
              return (
                <tr key={product.id} className="transition hover:bg-[#FAFBFC]">
                    ...
                    <div className="flex min-w-[260px] items-center gap-3">
                      <span className="relative flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-[#E5EAF0] bg-[#F7F9FB]">
                        {product.imageUrl ? (
                          ...
                          <PackageOpen className="size-5 text-[#94A3B8]" />
                        )}
                      </span>
                      <div className="min-w-0">
                        <p
                          className="max-w-[300px] truncate text-sm font-black text-[#172033]"
                          title={product.name}
                        >
                          {product.name}
                        </p>
                  ...
                  <td className="px-5 py-4">
                    ...
                    {product.salePrice && (
                      <p className="mt-1 text-xs font-semibold text-[#94A3B8] line-through">
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`inline-flex items-center gap-1.5 text-xs font-black ${product.isActive ? "text-emerald-700" : "text-[#64748B]"}`}
                    >
                      <span
                        className={`size-2 rounded-full ${product.isActive ? "bg-emerald-500" : "bg-[#94A3B8]"}`}
                      />
            ...
            {!filtered.length && (
              <tr>
                <td colSpan={8} className="px-5 py-16 text-center">
                  <AlertTriangle className="mx-auto size-7 text-[#94A3B8]" />
                  <p className="mt-3 text-sm font-bold text-[#64748B]">
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[#D8E0EA] bg-white shadow-2xl">
        ...
        <div className="flex justify-end gap-3 border-t border-[#E5EAF0] px-6 py-4">
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="rounded-lg border border-[#D8E0EA] px-4 py-2 text-sm font-black text-[#475569]"
          >
            Hủy
          </button>
          <button
...
function ActionGroup({
  ...
  return (
    <span className="block text-right text-xs font-black text-[#94A3B8]">
      Chỉ xem
    </span>
  );
}
  ...
  return (
    <Select value={value} onValueChange={onChange}>
      <SelectTrigger
        ...
        className={`h-10 rounded-xl border-border bg-background px-3 text-sm font-semibold shadow-none ${className}`}
      >
        <SelectValue />
      </SelectTrigger>
...
function MiniStat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="min-w-20 rounded-xl border border-[#E5EAF0] bg-[#F7F9FB] px-3 py-2.5 text-center">
      <p className="text-xl font-black tracking-normal text-[#172033]">
        {value}
      </p>
      <p className="text-[11px] font-black uppercase tracking-wider text-[#64748B]">
        {label}
  ...
  return [
    {
      key: "reason",
      label: "Lý do",
      render: (row: Row) => formatMatchingReportReason(row.reason),
    },
    {
      key: "targetType",
      label: "Đối tượng",
      render: (row: Row) => formatComplaintTarget(row.targetType),
    },
    { key: "targetId", label: "Người/Thú cưng bị phản ánh" },
    {
      key: "reporterId",
      label: "Người phản ánh",
      render: (row: Row) => row.reporter?.name ?? row.reporterId,
    },
...
async function loadMatchingReports(): Promise<{ data: Row[] }> {
  ...
  const matchingReports: Row[] = Array.isArray(matchingReportsResponse.data)
    ? matchingReportsResponse.data.map(
        (report: Row): Row => ({
          ...report,
...
function normalizeRows(section: string, data: Row[] | Row): Row[] {
  if (
    ["system-profile", "store-overview", "spa-overview"].includes(section) &&
    !Array.isArray(data)
  )
    return [data];
            ...
            <section className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
                  ...
                  <div className="mt-2 grid grid-cols-4 gap-2">
                    {profileImages.slice(1, 5).map((url, index) => (
                      <button
                        ...
                        className="relative aspect-square overflow-hidden rounded-lg border border-[#D8E0EA]"
                      >
                        <Image
                          src={url}
              ...
              <div className="grid content-start gap-4">
                ...
                <div className="rounded-xl border border-[#D8E0EA] bg-[#F7F9FB] p-4">
                  ...
                  <p className="mt-2 font-black text-[#172033]">
                    {detail.owner?.name ?? "-"}
                  </p>
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-3">
                    <div className="rounded-lg border border-[#E2E8F0] bg-white px-3 py-2.5">
                      <p className="mb-1.5 text-[10px] font-black uppercase tracking-wider text-[#64748B]">
                        Tài khoản chủ
                      </p>
                      <StatusBadge
                ...
                <div className="grid gap-3">
                  {documents.map((document) => {
                    ...
                    return (
                      <article
                        key={document.id}
                        className="rounded-xl border border-[#D8E0EA] p-4"
                      >
                          ...
                          >
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div className="min-w-0 flex-1">
                                ...
                                <p
                                  className="mt-0.5 truncate text-xs font-semibold text-[#64748B]"
                                  title={
                                    document.title ||
                                    formatDocumentType(document.type)
                                  }
                              ...
                              <div className="flex shrink-0 gap-2">
                                <button
                                  ...
                                  disabled={saving}
                                  className="rounded-lg border border-[#D8E0EA] bg-white px-3 py-2 text-xs font-black text-[#475569] disabled:opacity-50"
                                >
                                  Hủy
                                </button>
                                <button
...
function formatSpaWeightOption(min: number | null, max: number | null) {
  ...
  if (max == null || max === 100)
    return `${formatSpaWeightNumber(normalizedMin)}kg trở lên`;
