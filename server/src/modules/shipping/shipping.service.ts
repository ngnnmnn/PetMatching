...
export class ShippingService {
  ...
  async createShippingOrder(orderId: string) {
    ...
    try {
      ...
      // Hỗ trợ tự sửa lỗi (Self-healing) khi mã phường/xã bị sai lệch hoặc không hoạt động ở môi trường Sandbox/Dev
      if (
        (data.code === 400 &&
          data.message &&
          data.message.toLowerCase().includes('phường/xã')) ||
        (data.code !== 200 &&
          data.message &&
          data.message.toLowerCase().includes('ward'))
      ) {
        this.logger.warn(
          `Mã phường/xã ${body.to_ward_code} không hợp lệ hoặc đã dừng hoạt động. Đang tìm mã phường/xã thay thế cho district ${body.to_district_id}...`,
        );
        const fallbackWard = await this.getFirstActiveWardCode(
          body.to_district_id,
        );
        if (fallbackWard) {
          this.logger.warn(
            `Thử lại tạo vận đơn GHN với mã phường/xã hoạt động: ${fallbackWard}`,
          );
          body.to_ward_code = fallbackWard;
      ...
      if (data.code !== 200) {
        ...
        throw new BadRequestException(
          data.message || 'Không thể tạo đơn hàng trên GHN!',
        );
      }
  ...
  async handleWebhook(payload: any) {
    ...
    if (!order) {
      this.logger.warn(
        `Không tìm thấy đơn hàng trong hệ thống có ghnOrderCode=${ghnOrderCode}`,
      );
  ...
  async getShippingOrderDetail(ghnOrderCode: string) {
    try {
      const response = await fetch(
        `${this.ghnBaseUrl}/v2/shipping-order/detail`,
        {
          method: 'POST',
          headers: this.headers,
      ...
      if (data.code !== 200) {
        this.logger.error(
          `Không thể lấy chi tiết vận đơn GHN ${ghnOrderCode}: ${data.message}`,
        );
        return null;
      }
      return data.data;
      ...
      this.logger.error(
        `Lỗi kết nối GHN detail API cho mã ${ghnOrderCode}:`,
        error,
      );
      return null;
    }
