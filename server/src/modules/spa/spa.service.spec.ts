...
describe('SpaService staff checkin date restriction & notify late', () => {
  it('chặn check-in khi lịch hẹn thuộc ngày trong tương lai', async () => {
    ...
    await expect(service.staffCheckIn('staff-1', 'booking-future')).rejects.toThrow(
  ...
  it('gửi thông báo cho manager khi khách chưa đến và cập nhật trạng thái LATE', async () => {
    ...
    expect(createNotificationMock).toHaveBeenCalledWith(
      expect.objectContaining({
        userId: 'manager-1',
        title: expect.stringContaining('Khách chưa đến giờ hẹn'),
      }),
      expect.anything(),
    );
  });
...
describe('SpaService auto update overdue past bookings', () => {
  it('tự động chuyển các lịch quá ngày chưa hoàn thành sang COMPLETED với ghi chú nhân viên chưa hoàn thành lịch hẹn', async () => {
    const pastDate = new Date();
    pastDate.setDate(pastDate.getDate() - 3);

    const updateMock = jest.fn().mockResolvedValue({ status: 'COMPLETED' });
    ...
    expect(updateMock).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'booking-past-1' },
        data: expect.objectContaining({
          status: 'COMPLETED',
          note: 'Khách yêu cầu cắt ngắn (nhân viên chưa hoàn thành lịch hẹn)',
          petConditionAfter: 'Nhân viên chưa hoàn thành lịch hẹn',
        }),
      }),
    );
  });
